...
function main() {
  var f = File.openDialog('Kindly select your CSV file', false), docsData = [], delimiter = ',';
  if (!f)
    return;
...
function addPage(pagesLength) {
  if (pagesLength === void 0) { pagesLength = 0; }
  if (app.activeDocument) {
    ...
    if (page.side === PageSideOptions.LEFT_HAND) {
      ...
      dim.RIGHT =
        dim.WIDTH - parseFloat(doc.marginPreferences.left.toString());
    }
    else {
...
function addText(page, position, text) {
  if (app.activeDocument) {
    ...
    _tf.textFramePreferences.autoSizingType =
